...
export function gerarRelatorioPDF(sessao: Sessao, resultados: ResultadoSorteio[]) {
  ...
  const pk   = [253, 38, 193] as [number, number, number]
  const or   = [255, 144, 74] as [number, number, number]
  ...
  doc.text('CORRE COM ELAS · RUNNING CLUB', 14, 14)
  doc.setCharSpace(0)

  doc.setFontSize(22)
  doc.text('SORTE.IA CCE', 14, 26)

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(9)
  doc.setTextColor(255, 255, 255)
doc.text(`Evento: ${sessao.nomeEvento || 'Corre Com Elas'}`, 14, 35)
  ...
  autoTable(doc, {
    startY: 58,
    head: [['#', 'Sorteio', 'Nº Peito', 'Atleta', 'Desclassificados', 'Horário']],
  ...
  if (comDesclassificados.length > 0) {
    ...
    autoTable(doc, {
      startY: auditY + 4,
      head: [['Sorteio', 'Números Desclassificados']],
  ...
  doc.setTextColor(255, 255, 255)
  doc.setCharSpace(1)
  doc.text('SORTE.IA · RESULTADOS AUDITÁVEIS · CORRE COM ELAS', W / 2, finalY + 8.5, { align: 'center' })

  doc.save(`sorteio-cce-${Date.now()}.pdf`)
}
